...
const FindFollowers = () => {
  ...
  const followUser = (followingUserId) => {
    setFollowLoading(true);
    axiosInstance.post("/api/follow/", { followingUserId }).then((res) => {
      ...
      currentAuth.following.push(followingUserId);
      toggleAuth(currentAuth);
    });
  };

  const unfollowUser = (followingUserId) => {
    setUnfollowLoading(true);
    axiosInstance.post("/api/unfollow", { followingUserId }).then((res) => {
      ...
      currentAuth.following.forEach((followingUser, idx) => {
        if (followingUserId === followingUser._id) {
          authIndex = idx;
        }
      });
    ...
    <div className="container bg-gray-900 mx-auto mt-20 py-10">
          ...
          <Form autoComplete="off" className="px-10">
            <div className="mb-3 px-3 flex justify-center">
              <Field
                ...
                name="search"
                placeholder="search"
                className="rounded text-white bg-gray-800 px-2 py-2 transition duration-150 ease-in-out border border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent"
              />
              <button
                type="submit"
                className="bg-neon-purple inline rounded px-2 py-2 text-white disabled:opacity-50 ml-2"
                disabled={isSubmitting || !(isValid && dirty)}
              >
                {isSubmitting ? (
                  <ButtonSpinner className="w-5 animate-spin" />
                ) : (
      ...
      <div className="mt-6 sm:mt-10">
        {searchResults.map((user, idx) => (
          <div
            key={idx}
            className="py-2 px-3 bg-gray-800 rounded-full flex justify-between items-center w-full sm:w-3/4 md:w-3/5 lg:w-1/2 mx-auto mb-2"
          >
